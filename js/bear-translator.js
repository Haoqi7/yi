class BearTranslator {
    ...
    static #encodeChinese(text) {
        ...
        while (pos < totalLen) {
            ...
            if (!matched) {
                // 判断是否为最后一个需要编码的字符
                const isLastChar = pos === totalLen - 1;
                result.push({
                    text: this.#encodeBinary(text[pos], isLastChar),
                    type: 'encode'
                });
                pos++;
            }
    ...
    static #encodeBinary(char, isLast = false) {
        ...
        // 仅在非最后一个字符时添加分隔符
        return encoded + (isLast ? '' : this.#config.separator);
    }
